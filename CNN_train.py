...
def test_loop(model, train_dataloader, test_dataloader, metrics, args, checkpoint, 
              ...
              lamb, classLr, w_norm, w_lr, l_lr, b_lr, hsize, factor):
    ...
    with torch.no_grad():
        for inputs, targets in test_dataloader:
            ...
            if is_last_batch:
                ...
                csv_file_path = "/root/HebbianTopDown/CNN_hyper_search/AntiHebbFactor.csv"
                file_exists = os.path.isfile(csv_file_path)

                with open(csv_file_path, "a", newline="") as csvfile:
                    fieldnames = ["test_accuracy", "nlayers", "pool", "stride", "whiten", "triangle", "greedy", "inhib",
                                  "lambda", "classLr", "wnorm", "w_lr", "l_lr", "b_lr", "hsize", "AHfactor"]
...
def main(args, timer):
    ...
    test_loop(model, train_dataloader, test_dataloader, metrics, args, savedcheckpoint,
              ...
              hsize=config['Convolutions']['Layers']['Conv1']['out_channel'], factor=config["antihebbFactor"])
