...
from layers.softmax_sanger.ssang_classification_layer import SSangClassificationLayer
from layers.softmax_sanger.ssang_hebbian_layer import SSangHebbianLayer
from layers.softmax_sanger.ssang_input_layer import SSangInputLayer 


class SSangNetwork(Network):
    """
    CLASS
    ...
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        input_layer: InputLayer = SSangInputLayer()
        hebbian_layer: HiddenLayer = SSangHebbianLayer(self.input_dim, self.heb_dim, self.device, self.heb_param["lamb"], self.lr, self.heb_param["gam"], self.heb_param["eps"])
        classification_layer: OutputLayer = SSangClassificationLayer(self.heb_dim, self.output_dim, self.device, self.lr)
