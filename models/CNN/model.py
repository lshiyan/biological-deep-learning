...
class Hebbian_Layer(nn.Module):
    def __init__(self, inputdim, outputdim, lr, lamb, w_decrease, gamma, eps, is_output_layer=False, output_learning=ClassifierLearning.Supervised, update=Learning.FullyOrthogonal, weight=WeightScale.WeightDecay):
    ...
    def update_weights_OrthogonalExclusive(self, input: torch.Tensor, output: torch.Tensor):
        x: torch.Tensor = input.clone().detach().float().squeeze()
        x.requires_grad_(False)
        y: torch.Tensor = output.clone().detach().float().squeeze()
        y.requires_grad_(False)
        
        outer_prod: torch.Tensor = torch.einsum("i, j -> ij", y, x)
        
        weights: torch.Tensor = self.feedforward.weight.clone().detach()
        ...
        remove_diagonal: torch.Tensor = remove_diagonal.reshape(self.feedforward.weight.size(0), self.feedforward.weight.size(0), 1)
        scaled_weight: torch.Tensor = scaling_terms.reshape(self.feedforward.weight.size(0), self.feedforward.weight.size(0), 1) * weights.reshape(1, self.feedforward.weight.size(0), self.feedforward.weight.size(1))
    ...
    def update_weights(self, input, output, pred):
        if self.is_output_layer:
            if self.o_learning == ClassifierLearning.Contrastive:
                ...
                self.update_weights_FullyOrthogonal(input, pred)
...
class ConvolutionHebbianLayer(nn.Module):
    def __init__(self, input_size, kernel, stride, in_ch, out_ch, lambd, lr, gamma, epsilon, rho, weightlearning=Learning.FullyOrthogonal, weightscaling=WeightScale.WeightDecay):
        ...
        self.exponential_average=torch.zeros(self.out_channel)
    ...
    def update_weights_orthogonalexclusive(self, input: torch.Tensor, output: torch.Tensor):
        ...
        x: torch.Tensor = input.clone().detach().float()
        x.requires_grad_(False)
        y: torch.Tensor = output.clone().detach().float()
        y.requires_grad_(False)
        ...
        remove_diagonal: torch.Tensor = remove_diagonal.reshape(weights.size(0), weights.size(0), 1)

        scaled_weight: torch.Tensor = scaling_terms.reshape(weights.size(0), weights.size(0), 1) * weights.reshape(1, weights.size(0), weights.size(1))

        scaled_weight: torch.Tensor = scaled_weight * remove_diagonal

        norm_term: torch.Tensor = torch.einsum("ai, ak, ikj -> ij", y, y, scaled_weight)

        computed_rule: torch.Tensor = (outer_prod - norm_term)
        ...
        # print("Delta")
        # print(avg_delta_weight)

        avg_delta_weight = self.lr*avg_delta_weight
...
def CNNBaseline_Model(inputsize, kernel, stride, inchannel, outchannel, lambd, lr, gamma, epsilon, rho, nbclasses, topdown, wl, ws, o):

    mycnn = ConvolutionalNeuralNet()
    convlayer1 = ConvolutionHebbianLayer(input_size=inputsize, kernel=kernel[0], stride=stride[0], in_ch=inchannel[0], out_ch=outchannel[0], lambd=lambd, lr=lr, gamma=gamma, epsilon=epsilon, rho=rho, weightlearning=wl, weightscaling=ws)
    ...
    convlayer2 = ConvolutionHebbianLayer(input_size=hinputsize, kernel=kernel[1], stride=stride[1], in_ch=inchannel[1], out_ch=outchannel[1], lambd=lambd, lr=lr, gamma=gamma, epsilon=epsilon, rho=rho, weightlearning=wl, weightscaling=ws)
    ...
    convlayer3 = ConvolutionHebbianLayer(input_size=hinputsize2, kernel=kernel[2], stride=stride[2], in_ch=inchannel[2], out_ch=outchannel[2], lambd=lambd, lr=lr, gamma=gamma, epsilon=epsilon, rho=rho, weightlearning=wl, weightscaling=ws)
    ...
    heblayer = Hebbian_Layer(inputdim=fc_inputdim, outputdim=nbclasses, lr=lr, lamb=lambd, w_decrease=10, gamma=gamma, eps=epsilon, is_output_layer=True, output_learning=o)
...
def view_filters(v_model, folder):
    w1 = v_model.layers['CNNLayer1'].convolution.weight
    w2 = v_model.layers['CNNLayer2'].convolution.weight
    w3 = v_model.layers['CNNLayer3'].convolution.weight
...
def view_classifier(v_model, file):
    ...
    weight = v_model.layers['HebLayer'].feedforward.weight
...
def view_ff_weights(v_model, folder, loader, save=True):
    inputs, data = next(iter(loader))
    inputs = inputs.reshape(1,1,28,28)
    ...
    view_map(fmaps, folder + '/Fmap1.png', save)
    view_map(fmaps2, folder + '/Fmap2.png', save)
    view_map(fmaps3, folder + '/Fmap3.png', save)


def view_TD_weights(self, folder, loader, save=True):
    input, data = next(iter(loader))
    ...
    for idx in range(len(hlist)-1):
        view_map(hlist[idx], folder + '/Fmap' + str(idx+1) + '.png', save)
    

def view_map(fmaps, file, save):
    if fmaps.size(1) == 1:
        plt.imshow(fmaps[0][0], interpolation='nearest')
        plt.colorbar()
        if save:
